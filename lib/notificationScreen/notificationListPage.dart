...
class _NotificationListPageState extends State<NotificationListPage> {
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      length: userType == "doctor" ? 1 : userType == "pharma"?1:userType == "labattender"?1: 2,
      child: Scaffold(
        appBar: AppBar(
        ...
        bottomNavigationBar: userType == "doctor"
        ||userType=="pharma"||userType=="labattender"
            ? null
            : BottomNavBarWidget(
                title: "Send to",
  ...
  _admin() {
    return _isLoading
        ...
        : FutureBuilder(
            future:
            userType == "labattender"
                ? AdminNotificationService.getDataByLabAttenderId(_adminLimit, laId):
            userType == "doctor"
                ? AdminNotificationService.getDataByDoctId(_adminLimit, doctId)
            :            userType == "pharma"
                ? AdminNotificationService.getDataByPharmaId(_adminLimit, pharmaId)
  ...
  void _setRedDot() async {
    ...
    if (userType == "doctor")
      ...
      await UpdateData.updateIsAnyNotification("pharma", pharmaId, false);
    else if(userType=="labattender")
      await UpdateData.updateIsAnyNotification("labattender", laId, false);
    else
  ...
  void _getAndSetData() async {
    ...
    if (userType == "doctor") {
      ...
      pharmaId= prefs.getString("pharmaId") ?? "";
    else if(userType=="labattender") {
      laId = prefs.getString("laId") ?? "";
      laClinicId = prefs.getString("lacId") ?? "";
    }
    _setRedDot();
  }
}
