...
class ClinicService {
  ...
  static const _clinicByIdUrl = "$apiUrl/get_clinic_byid";
  static const _getAllUrl = "$apiUrl/get_all_clinic";
  ...
  static Future<List<ClinicModel>> getDataByClinicId(String clinicID) async {
    final response =
        await http.get(Uri.parse("$_clinicByIdUrl?clinicId=$clinicID"));
    if (response.statusCode == 200) {
      List<ClinicModel> list = dataFromJson(response.body);
      return list;
    } else {
  ...
  static Future<List<ClinicModel>> getAllData() async {
    final response = await http.get(Uri.parse("$_getAllUrl"));
    if (response.statusCode == 200) {
      List<ClinicModel> list = dataFromJson(response.body);
      return list;
    } else {
