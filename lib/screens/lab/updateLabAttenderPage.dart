...
class EditLabAttenderPage extends StatefulWidget {
  final clinicId;
  final dataDetails;
  const EditLabAttenderPage({Key? key, this.dataDetails, this.clinicId})
      : super(key: key);

  @override
  _EditLabAttenderPageState createState() => _EditLabAttenderPageState();
}

class _EditLabAttenderPageState extends State<EditLabAttenderPage> {
  ...
  void initState() {
    ...
    titleController.text = widget.dataDetails.name;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: IAppBars.commonAppBar(context, "Edit Lab Attended"),
  ...
  _handleDeleteService() async {
    ...
    final res = await LabAttenderService.deleteData(widget.dataDetails.id);
    // print(res);
    if (res == "success") {
  ...
  _updateDetails(imageDownloadUrl) async {
    LabAttenderModel labAttenderModel = LabAttenderModel(
    ...
    final res = await LabAttenderService.updateData(labAttenderModel);
