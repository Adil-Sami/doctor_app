...
class AddLabAttenderPage extends StatefulWidget {
  final clinicID;
  final clinicName;
  AddLabAttenderPage({this.clinicID, this.clinicName});
  @override
  _AddLabAttenderPageState createState() => _AddLabAttenderPageState();
}

class _AddLabAttenderPageState extends State<AddLabAttenderPage> {
  bool passValid = true;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: IAppBars.commonAppBar(context, "Add Lab Attender"),
  ...
  Widget _serviceNameInputField() {
    return InputFields.commonInputField(_nameController, "Name*", (item) {
  ...
  _uploadData(imageDownloadUrl) async {
    LabAttenderModel labAttenderModelAtt = LabAttenderModel(
    ...
    final res2 = await LabAttenderService.addData(labAttenderModelAtt);
    if (res2 == "error") {
